...
$__dataDir = __DIR__ . '/source/_data';

// Load an API snapshot committed under source/_data/api (generated by
// scripts/refresh-template-data.php). Falls back to the live API only if the
// snapshot is missing — so a fresh clone still builds, but normal builds make
// no network call. The app API remains the upstream source of truth for which
// templates exist; this just materializes it into the repo.
$loadApi = function (string $file, string $url) use ($__dataDir) {
    $path = "$__dataDir/api/$file";
    if (is_file($path)) {
...
// Per-template SEO + image overrides, owned in THIS repo and keyed by
// template_slug (see source/_data/template-overrides.json). This is where you
// enrich a template page or swap its image without touching the app/DB.
$templateOverrides = is_file("$__dataDir/template-overrides.json")
    ...
    'collections' => [
        'template' => [
            ...
            'items' => function ($config) use ($loadApi, $templateOverrides) {
                 $templates = $loadApi('templates.json', $config['appUrl'].'/api/templates');

                 return collect($templates)->map(function ($template) use ($templateOverrides) {
                    $ov = $templateOverrides[$template->template_slug] ?? [];
                    return [
                        'slug' => $template->slug,
                        'type' => $template->type,
                        'categories' => $template->categories,
                        'category_slug' => Str::slug($template->categories[0]),
                        // title/description drive the <head>; heading is the on-page H1.
                        'title' => $ov['meta_title'] ?? $template->template_title,
                        'heading' => $template->template_title,
                        'template_slug' => $template->template_slug,
                        'description' => $ov['meta_description'] ?? $template->template_description,
                        'image' => $ov['image'] ?? $template->template_image,
                        // Flatten enrichment to top-level keys — Jigsaw only reliably
        ...
        'type' => [
            ...
            'items' => function ($config) use ($loadApi, $templateOverrides) {
                 $types = $loadApi('types.json', $config['appUrl'].'/api/types');

                 return collect($types)->map(function ($type_obj) use($config, $templateOverrides) {
                    ...
                    $data['categories'] = collect($type_obj->categories)->map(function ($category) use ($templateOverrides) {
                        return [
                            'name' => $category->name,
                            'slug' => $category->slug,
                            'templates' => collect($category->templates)->map(function ($template) use ($templateOverrides) {
                                $ov = $templateOverrides[$template->template_slug] ?? [];
                                return [
        ...
        'form_category' => [
            ...
            'items' => function ($config) use ($loadApi, $templateOverrides, $applyCategoryOverrides) {
                 ...
                 return collect($categories)->map(function ($category_obj) use($config, $templateOverrides, $applyCategoryOverrides) {
                    ...
                    $data['templates'] = collect($category_obj->templates)->map(function ($template) use ($templateOverrides) {
                        $ov = $templateOverrides[$template->template_slug] ?? [];
                        return [
                            'slug' => $template->template_slug,
                            'image' => $ov['image'] ?? $template->image,
